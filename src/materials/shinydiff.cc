...
CFLOAT shinyDiffuseMat_t::OrenNayar(const vector3d_t &wi, const vector3d_t &wo, const vector3d_t &N) const
{
	...
	if(cos_to >= cos_ti)
	{
        sin_alpha = fSqrt(std::max(0.f, 1.f - cos_ti*cos_ti));
        tan_beta = fSqrt(std::max(0.f, 1.f - cos_to*cos_to)) / std::max(1e-8f, cos_to);
	}
	else
	{
        sin_alpha = fSqrt(std::max(0.f, 1.f - cos_to*cos_to));
        tan_beta = fSqrt(std::max(0.f, 1.f - cos_ti*cos_ti)) / std::max(1e-8f, cos_ti);
	}
